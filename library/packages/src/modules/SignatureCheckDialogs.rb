  ...
  class SignatureCheckDialogsClass < Module
      ...
      UI.OpenDialog(
        ...
        VBox(
          HBox(
            ...
            VCenter(
              Heading(
                item_type == :package ? _("Unsigned Package") : _("Unsigned File")
              )
            ),
            HStretch()
          ),
          ...
          Left(
            MarginBox(
              ...
              CheckBox(
                ...
                !GetShowThisPopup(dont_show_dialog_ident, item_name)
              )
            )
          ),
          YesNoButtons(:no)
    ...
    def ItemSignedWithUnknownSignature(item_type, item_name, key_id, dont_show_dialog_ident, repoid)
      ...
      description_text = Builtins.sformat(
        ...
        repo_url != "" ? Builtins.sformat("%1 (%2)", item_name, repo_url) : item_name,
        Ops.add(
          "\n",
          # Part of the GnuPG key description in popup, %1 is a GnuPG key ID
    ...
    def ImportGPGKeyIntoTrustedDialog(key, repository)
      ...
      UI.OpenDialog(
        ...
        HBox(
          # left-side help
          hide_help ? Empty() : HWeight(3, VBox(RichText(warning_text))),
          HSpacing(1.5),
          # dialog
          HWeight(
            5,
